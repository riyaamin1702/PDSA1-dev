...
the data array, and validate columns 6,8,9,11 of every
...
def validate_all_scores (data_array):
  ...
  for line in data_array:
    ...
    if(len(line) > 3):
      ...
      if(computeScore[0] != line[4]):
        line[4] = computeScore[0]
      ...
      if(computeScore[1] != line[6]):
        line[6] = computeScore[1]
      ...
      if(md_conv_Total != line[8]):
        line[8] = md_conv_Total
      ...
      if (computed_Total > 0):
        ...
        lose_Result = ' L '
        if(lose_Result != line[7]):
          line[7] = lose_Result
          print(f"Incorrect value found in Round {line[0]} during Result validation. The incorrect value was {result} and it has been updated to {line[7]}")
      else:
        draw_result = ' D '
        if(draw_result != line[7]):
          line[7] = draw_result
          print(f"Incorrect value found in Round {line[0]} during Result validation. The incorrect value was {result} and it has been updated to {line[7]}")
